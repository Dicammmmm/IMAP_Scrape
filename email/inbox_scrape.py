...
def decode_header_safe(header) -> str:
    ...
    try:
        ...
        return result
    except Exception: return str(header) # Fallback

def get_sqlalchemy_type(dtype):
    """Maps pandas dtype to SQLAlchemy type."""
    if pd.api.types.is_integer_dtype(dtype): return BigInteger if dtype.name == 'int64' else Integer
...
def get_sqlalchemy_engine():
    ...
    try:
        ...
        return engine
    except Exception as e: log.error(f"SQLAlchemy engine failed: {e}"); raise
...
def test_imap_connection() -> bool:
    ...
    try:
        ...
        if response == 'OK': imap_conn.logout(); return True
        else: log.error(f"IMAP login failed: {response}"); return False
    except Exception as e: log.error(f"IMAP connection error: {e}"); return False
...
def fetch_brand_sender_pairs(engine) -> list:
    ...
    try:
        ...
        return pairs
    except Exception as e: log.error(f"Error fetching from 'emails' table: {e}"); return []

# ---------- Email Processing Logic ----------

def process_inbox(expected_sender: str):
    """Connects to IMAP, finds emails FROM sender, extracts attachments."""
    if not expected_sender: log.error("process_inbox: expected_sender missing."); return []
    processed_data = []
    ...
    try:
        ...
        if status != 'OK' or not messages or not messages[0]:
            log.info(f"IMAP: No emails found for search: {search_criteria}"); return []
        ...
        for num in email_ids:
            ...
            try:
                ...
                if parsed_sender != expected_sender: continue # Double check sender
                    ...
                    if part.get_content_disposition() and part.get_content_disposition().startswith("attachment"):
                        ...
                        if filename.lower().endswith(('.csv', '.xlsx', '.xls')):
                            ...
                            if payload:
                                try:
                                    df = None
                                    if filename.lower().endswith('.csv'): df = pd.read_csv(io.BytesIO(payload))
                                    elif filename.lower().endswith('.xlsx'): df = pd.read_excel(io.BytesIO(payload), engine='openpyxl')
                                    elif filename.lower().endswith('.xls'): df = pd.read_excel(io.BytesIO(payload), engine='xlrd')
                                    if df is not None and not df.empty: attachment_dfs.append(df)
                                except Exception as read_err: log.error(f"Read error: '{filename}' in email {msg_id}: {read_err}")
                if attachment_dfs:
    ...
    except Exception as e: log.exception(f"Error in process_inbox for {expected_sender}: {e}"); return []
    finally:
        if imap_conn:
...
def save_dataframe_to_db(msg_id: str, sender: str, subject: str, df: pd.DataFrame, engine, target_table: str) -> str:
    ...
    if not msg_id or msg_id.startswith('missing_id_'): log.warning(f"Unreliable msg_id '{msg_id}', skipping save."); return 'failed'
    if not target_table or not engine: log.error("Missing target_table or engine."); return 'failed'
    ...
    try:
        ...
        for i, col in enumerate(original_columns):
            ...
            if not col_str: col_str = f"unnamed_col_{i}"
        ...
        if inspector.has_table(target_table):
            ...
            if 'content_hash' in table_columns:
                try:
                    ...
                    with engine.connect() as conn: result = conn.execute(check_sql, {'hash': content_hash}).scalar()
                    if result == 1: log.info(f"Skipping msg {msg_id}: Duplicate content hash {content_hash[:10]}... found."); return 'skipped_hash'
                except Exception as e: log.warning(f"Error checking content_hash in {target_table}: {e}")
            ...
            if 'message_id' in table_columns:
                try:
                    ...
                    with engine.connect() as conn: result = conn.execute(check_sql, {'msg_id': msg_id}).scalar()
                    if result == 1: log.info(f"Skipping msg {msg_id}: Duplicate message_id found."); return 'skipped_msg_id'
                except Exception as e: log.warning(f"Error checking message_id in {target_table}: {e}")
        ...
        for col_name in df_processed.columns:
            ...
            if col_name == 'message_id' or col_name == 'content_hash': sql_type = Text # Ensure Text for IDs/Hashes
        ...
        except Exception as e: log.error(f"Table creation/check error for '{target_table}': {e}"); return 'failed'
...
def main():
    ...
    try:
        ...
        if not test_imap_connection(): raise ConnectionError("IMAP connection failed.")
        log.info("Connections established.")

        tasks = fetch_brand_sender_pairs(db_engine)
        if not tasks: log.info("No sender/brand tasks found in 'emails' table."); return
    ...
    except Exception as e: log.exception(f"Critical error in main: {e}")
    finally:
        if db_engine: db_engine.dispose(); log.info("DB Engine disposed.")
